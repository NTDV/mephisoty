...
import { MathsStageService } from '@/service/admin/stages/MathsStageService';
...
const mathStageService = new MathsStageService();
...
const loadLazyData = (event) => {
  ...
  mathStageService.getAll(lazyParams.value).then((data) => {
        ...
        <DataTable
          ...
          <template #header>
            <div class="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
              <h5 class="m-0">Управление участниками Математических боев</h5>
          ...
          <Column field="title" header="Команда" headerStyle="width: 20%; min-width:10rem;">
            <template #body="slotProps">{{ slotProps.data.title }}</template>
          </Column>

          <Column field="captain" header="Капитан" headerStyle="width:15%; min-width:10rem;">
            <template #body="slotProps">{{ slotProps.data.captain }}</template>
          </Column>

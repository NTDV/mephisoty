...
const loadLazyData = (event) => {
    ...
    setTimeout(
        () =>
            stageService.getAll(lazyParams.value).then((data) => {
...
<template>
    <div class="grid">
        <div class="col-12">
            <div class="card">
                <Dropdown v-model="parentSeason" :options="seasonSelectData" class="p-column-filter" option-label="title"
                          option-value="id" placeholder="Выберите сезон" @change="clearFilter">
                ...
                <Toolbar class="mb-4">
                    <template v-slot:start>
                        <div class="my-2">
                            <Button class="mr-2" icon="pi pi-plus" label="Добавить" severity="success"
                                    @click="openNew"/>
                            <Button :disabled="!selectedStages || !selectedStages.length" icon="pi pi-trash" label="Удалить" severity="danger"
                                    @click="confirmDeleteSelected"/>
                        </div>
                    </template>

                    <template v-slot:end>
                        <Button icon="pi pi-upload" label="Экспорт" severity="help" @click="exportCSV($event)"/>
                    </template>
                </Toolbar>
                    ...
                    <Column :showFilterMatchModes="false" :sortable="true" field="seasonVisibility"
                        ...
                        <template #filter="{ filterModel, filterCallback }">
                            <Dropdown v-model="filterModel.value" :options="statuses" class="p-column-filter"
                                      ...
                                      @keydown.enter="filterCallback()">
                                <template #option="slotProps">
                                    <Tag :key="slotProps.option.value" :severity="allowStateService.getBadgeSeverityFor(slotProps.option.value)"

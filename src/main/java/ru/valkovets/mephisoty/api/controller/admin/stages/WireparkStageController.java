...
import ru.valkovets.mephisoty.db.service.stages.WireparkStageService;
import ru.valkovets.mephisoty.db.service.userdata.UserService;

@RestController
@RequestMapping("/api/admin/stages/wirepark")
@RequiredArgsConstructor
@Tag(name = "Веревочный парк")
public class WireparkStageController {
private final WireparkStageService wireparkStageService;
private final UserService userService;

@PostMapping("/")
@Operation(summary = "Получить информацию о выбранном времени участников")
public GetAllDto<UserScheduleRecordDto> getAll(@RequestBody final DataTablePageEvent searchParams) {
  return GetAllDto.from(
      wireparkStageService.getAll(
...
public void update(@PathVariable final Long userId, @PathVariable final Long scheduleId) {
  userService.chooseWireparkDate(userId, scheduleId);
}
}

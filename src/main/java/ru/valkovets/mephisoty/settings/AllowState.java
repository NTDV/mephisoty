...
public boolean isCanEdit() throws AccessDeniedException, IllegalStateException {
  if (equals(AllowState.DISALLOW_ALL_FOR_PARTICIPANTS) ||
      equals(AllowState.ALLOW_READ_FOR_PARTICIPANTS)) {
    throw new AccessDeniedException("Нельзя выбрать дату");
  } else if (equals(AllowState.ALLOW_CREATE_READ_FOR_PARTICIPANTS)) {
    return false;
  } else if (equals(AllowState.ALLOW_CREATE_EDIT_READ_FOR_PARTICIPANTS)) {
    return true;
  } else {
    throw new IllegalStateException("Unknown scheduleAccess: " + this);
  }
}
}